...
def convert_from_path(pdf_path, dpi=200, output_folder=None, first_page=None, last_page=None, fmt='ppm', thread_count=1, userpw=None, use_cropbox=False, strict=False, transparent=False, output_file=str(uuid.uuid4())):
    ...
    for i in range(thread_count):
        ...
        if i < 10:
            ...
            pg = 'page' + str(i)
        thread_output_file = pg + '(' + output_file + ')' if thread_count > 1 else output_file 
        ...
        if platform.system() != 'Windows':
            ...
            processes.append((thread_output_file, Popen(args, stdout=PIPE, stderr=PIPE, creationflags=134217728)))        
    ...
    try:
        if userpw is not None:
            if platform.system() != 'Windows':
                ...
                proc = Popen(["pdfinfo", pdf_path, '-upw', userpw], stdout=PIPE, stderr=PIPE, creationflags=134217728)
        else:
            if platform.system() != 'Windows':
                proc = Popen(["pdfinfo", pdf_path], stdout=PIPE, stderr=PIPE)
            else:
                proc = Popen(["pdfinfo", pdf_path], stdout=PIPE, stderr=PIPE, creationflags=134217728)
